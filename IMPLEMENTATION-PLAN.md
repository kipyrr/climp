...
| **D24** | How the shortcuts invoke the app | → **a launcher script, not `-m climp.main`.** The module form resolves the package against the current working directory, so it only worked when the process started in the repo. Explorer did not supply that directory, and the resulting `ModuleNotFoundError` went to a process with no console, before logging existed -- a completely silent failure that took three rounds to find. `climp_launcher.pyw` sets `sys.path` from its own location, writes `launch.log` before anything can fail, and reports any error in a dialog. | **Settled** |
| **D23** | Whether more than one copy may run | → **no.** A named mutex (`Local\climp-single-instance`) guards startup; a second launch shows a message box saying it is already running and where to find the tray icon, then exits. Found in use: the tray icon was hidden in the Windows overflow area, so a successful start was indistinguishable from a failure, and repeated double-clicks stacked up four processes on one folder and one database. Nothing corrupted — the row claim is atomic — but it wasted bandwidth and was very hard to diagnose. | **Settled** |
| **D25** | Whether the icon pulses while work is merely queued | → **no.** Revises D21. The pulse now requires work that can actually progress: nothing animates while uploads are paused for a fullscreen game or a broken sign-in. Two reasons, both found in use. It misleads -- a pulsing icon says something is happening when nothing can. And it is not free: each frame is a tray-icon update, ten a second, for as long as the pause lasts, which made the menu lag. | **Settled** |
